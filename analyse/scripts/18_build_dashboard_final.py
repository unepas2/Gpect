...
// charts
function hbar(id,labels,data,color,max){new Chart(document.getElementById(id),{type:'bar',data:{labels,datasets:[{data,backgroundColor:color,borderRadius:4,maxBarThickness:24}]},options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,scales:{x:{beginAtZero:true,max:max||undefined,grid:{color:'#eef2f7'}},y:{grid:{display:false},ticks:{autoSkip:false,font:{size:10.5}}}},plugins:{tooltip:{callbacks:{label:c=>' '+fmt(c.parsed.x)}}}}});}
function vbar(id,labels,data,colors,max){new Chart(document.getElementById(id),{type:'bar',data:{labels,datasets:[{data,backgroundColor:colors,borderRadius:5,maxBarThickness:64}]},options:{responsive:true,maintainAspectRatio:false,scales:{y:{beginAtZero:true,max:max||5,grid:{color:'#eef2f7'}},x:{grid:{display:false}}},plugins:{tooltip:{callbacks:{label:c=>' '+fmt(c.parsed.y)}}}}});}
...
// auto render by type
function auto(c,k){const d=Q(c,k);if(!d)return '';const cm=COMMENTS[c+'|'+k]||'';const t=d.type,id=uid(c,k),col=COL[c],title=lbl(d.intitule);
 if(t==='echelle_1_5'){const sf=d.sans_financeur?`<br>sans le financeur : <b>${fmt(d.sans_financeur.moyenne)}</b> (n=${d.sans_financeur.n})`:'';
   return card(title,'Échelle de 1 à 5',`<div class="bigrow"><div class="big">${fmt(d.moyenne)}<span>/5</span></div><div class="bigmeta">médiane ${fmt(d.mediane)} · min ${d.min} · max ${d.max}<br>n=${d.n}${sf}</div></div>`,cm);}
 if(t==='numerique'){const s2=d.sans_plus_gros?`<br>sans le plus gros employeur : <b>${fmt(d.sans_plus_gros.somme)}</b> (méd. ${fmt(d.sans_plus_gros.mediane)})`:'';
   const nn=d.n_non_numerique_exclus?` · ${d.n_non_numerique_exclus} non chiffrable(s)`:'';
   return card(title,'Numérique',`<div class="bigrow"><div class="big">${fmt(d.somme)}</div><div class="bigmeta">total · médiane ${fmt(d.mediane)} · n=${d.n}${nn}${s2}</div></div>`,cm);}
 if(t==='choix_unique'){const m=d.modalites;CQ.push(()=>dough(id,m.map(x=>shorten(x.modalite,30)),m.map(x=>x.n),PAL));
   return card(title,'Répartition · n='+d.n_repondants,chCanvas(id),cm);}
 if(t==='multi_select'){const a=d.modalites.filter(x=>x.n>0);CQ.push(()=>hbar(id,a.map(x=>x.modalite),a.map(x=>x.pct_repondants),col,100));
   return card(title,'% des répondants (choix multiple) · n='+d.n_repondants,chCanvas(id,a.length>6),cm);}
